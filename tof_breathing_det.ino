...
short int min_distance;
short int center_distance;
const int KT = pow(2,7);
int data[KT];
...
float m_peaks[5];   // top 5 magnitude peaks in descending order
...
void loop()
{
  ...
  if (myImager.isDataReady() == true)
  {
    if (myImager.getRangingData(&measurementData)) //Read distance data into array
    {
      ...
      //      Serial.println(measurementData.distance_mm[2 + 2]);

      //      min_distance = min_array(measurementData.distance_mm);

//      center_distance = measurementData.distance_mm[(imageWidth - 1) * imageWidth / 2];
      center_distance = 0.25*(measurementData.distance_mm[5]+measurementData.distance_mm[6]+measurementData.distance_mm[9]+measurementData.distance_mm[10]);

      memmove(&data[0], &data[1], sizeof(data) - sizeof(*data));
      data[KT - 1] = center_distance;
...
//      Serial.println();
      ...
      FFT(data, KT, 15);      //to   get top five value of frequencies and magnitudes of X having KT sample at 15Hz sampling
      Serial.print(m_peaks[0]); Serial.print('\t');
      Serial.print(m_peaks[1]); Serial.print('\t');
      Serial.print(f_peaks[0]); Serial.print('\t');
      Serial.println();

//      for (int i = 0 ; i < 5 ; i++) {
//        Serial.print(m_peaks[i]);
//        Serial.print('\t');
//      }
//      Serial.println(); 
//      for (int i = 0 ; i < 5 ; i++) {
...
//      Serial.println(); 
//      Serial.println();      
      if (f_peaks[0] < 0.5 && f_peaks[0] > 0.2){
        if (m_peaks[0]/m_peaks[1] > 1.8 && m_peaks[0]<300 && m_peaks[0]>10){
          breathing = true;
        }
        else{
          breathing = false;
